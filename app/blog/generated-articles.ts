...
import { august20Articles } from './august20-batch';
import { august21Articles } from './august21-batch';
import { august23Articles } from './august23-batch';
...
import { september4Articles } from './september4-batch';
...
const generatedArticleShells: Record<string, RichArticle> = Object.fromEntries(allSeeds.map((seed, index) => {
  ...
  return [seed.slug, {
    ...
    directAnswer: [`For ${seed.focus}, a Philippines-based assistant can prepare the approved queue, check required fields, and document each exception while the HR owner keeps policy, employee-relations, and approval decisions.`, `Begin ${seed.focus} with one named queue, the specific tools in the brief, an assigned reviewer, and completion evidence that the owner can compare with the approved process.`],
